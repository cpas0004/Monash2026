...
class PacketSink(object):
    ...
    """
    def __init__(self, env, rec_arrivals=True, absolute_arrivals=True, rec_waits=True, debug=False, selector=None):
...
class SwitchPort(object):
    ...
    qlimit : integer (or None)
        ...
        in service).
    limit_bytes : bool
        If True, the queue limit is based on bytes; otherwise it is based on packets.
    """
    ...
    def __init__(self, env, rate, qlimit=None, limit_bytes=True, debug=False):
        ...
        self.packets_rec = 0
        self.packets_drop = 0
        self.packets_drop_A = 0
        self.packets_drop_B = 0
        self.qlimit = qlimit
    ...
    def put(self, pkt):
        ...
        if self.limit_bytes and tmp_byte_count >= self.qlimit:
            ...
            if hasattr(pkt, "src"):
                if pkt.src == "Source A":
                    self.packets_drop_A += 1
                    self.dropped_packets_A.append(pkt)
                elif pkt.src == "Source B":
                    self.packets_drop_B += 1
